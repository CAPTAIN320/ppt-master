...
- Binds port 5050 by default. Step 4 always shuts this server down on exit (see `--shutdown`) so the port is free for other tools. Keeps a **separate process and lock** (`.confirm_ui.lock`).
...
- No SVG / layout preview here — that is handled separately.
